...
def _init_pool_worker():
    ...
    """
    global am
    from argoverse.map_representation.map_api import ArgoverseMap as _PoolAM
    am = _PoolAM()
    import sys
    sys.stderr.flush()
...
def _pool_load_file(file_and_args):
    ...
    try:
        ...
        _pool_load_file._err_count += 1
        if _pool_load_file._err_count <= 20:
            import sys
            print(f'[PoolError] file={file} err={type(e).__name__}: {e}', file=sys.stderr, flush=True)
            import traceback
